...
public class Cookies {//쿠키 클래스
	...
	private Map<String,Cookie> cookieMap = new HashMap<>();//문자열 쿠키이름,값으로 쿠키객체를
	//쌍으로 저장하는 사전적인 자료구조 컬렉션 맵
	
	public Cookies(HttpServletRequest request) {//HttpServletRequest 서블릿은 사용자폼에서 
		//입력한 정보를 서버로 가져오는 역할을 한다.
		Cookie[] cookies = request.getCookies();//쿠키배열을 구함
		if(cookies != null) {
	...
	throws IOException{//전달인자 개수가 다른 메서드 오버로딩
