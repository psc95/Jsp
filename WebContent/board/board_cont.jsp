...
<%@ page import="net.daum.vo.*,net.daum.dao.*" %><%--복수개의 패키지를 import --%>
<%
	int bno=Integer.parseInt(request.getParameter("bno"));//get으로 전달된 번호값을 정수 숫자로
	//변경해서 저장
 ...
 <table border="1">
  ...
  <tr>
   ...
   <input type="button" value="수정" onclick="location=
	   'board25_edit.jsp?bno=<%=bc.getBno()%>';" >
   <input type="button" value="삭제" onclick="location=
	   'board25_del.jsp?bno=<%=bc.getBno()%>';" >
   <%-- board_del.jsp?bno=번호를 주소창에 노출되는 get방식으로 전달함. --%>
