...
<%
	...
	String bname = request.getParameter("bname"); //글쓴이 저장
	String btitle = request.getParameter("btitle");//글제목 저장
	String bcont = request.getParameter("bcont"); //글내용 저장
	...
	try{
		...
		sql = "insert into boardT (bno,bname,btitle,bcont,bdate) values("
				+"bno_seq25.nextval,'"+bname+"','"+btitle+"','"+bcont+"', sysdate)";
		...
		if(re == 1){
			...
			out.println("location='board_list.jsp';");//게시판 목로보기로 이동
			out.println("</script>");
		}
	}catch(Exception e){
		e.printStackTrace();
